...
// fillToken returns the fill attribute for a color: a class reference when the
// color is in the palette, otherwise an inline fill.
func (r *Renderer) fillToken(color string) string {
	if cls, ok := r.classOf[color]; ok {
		return ` class="` + cls + `"`
	}
	return ` fill="` + color + `"`
}
...
`
	...
	if _, err := fmt.Fprintf(r.w, template, formatFloat(width), formatFloat(height), formatFloat(width), formatFloat(height), r.palette.background, html.EscapeString(r.cfg.FontFamily), formatFloat(r.cfg.FontSize)); err != nil {
		return err
	}
	_, err := io.WriteString(r.w, r.styleBlock+r.nl)
	return err
}
...
func (r *Renderer) emitRowBackground(row int) error {
	...
	_, err := fmt.Fprintf(r.w, `<rect x="%s" y="%s" width="%s" height="%s"%s/>`+r.nl,
		formatFloat(x), formatFloat(y), formatFloat(width), formatFloat(r.cfg.CellHeight), r.fillToken(r.palette.background))
	return err
}
...
func (r *Renderer) emitBackgrounds(row int, cells []terminal.Cell) error {
	for col := 0; col < len(cells); {
		...
		if _, err := fmt.Fprintf(r.w, `<rect x="%s" y="%s" width="%s" height="%s"%s/>`+r.nl,
			formatFloat(x), formatFloat(y), formatFloat(width), formatFloat(r.cfg.CellHeight), r.fillToken(bg)); err != nil {
			return err
		}
	}
	return nil
...
func (r *Renderer) emitText(row int, cells []terminal.Cell) error {
	for col := 0; col < len(cells); {
		...
		if _, err := fmt.Fprintf(r.w, `" y="%s"%s`, formatFloat(y), r.fillToken(key.fg)); err != nil {
			return err
		}
		if key.bold {
...
func (r *Renderer) emitCursor(row int, cells []terminal.Cell) error {
	...
	y := r.cfg.Padding + float64(row)*r.cfg.CellHeight
	if _, err := fmt.Fprintf(r.w, `<rect x="%s" y="%s" width="%s" height="%s"%s/>`+r.nl,
		formatFloat(x), formatFloat(y), formatFloat(r.cfg.CellWidth), formatFloat(r.cfg.CellHeight), r.fillToken(r.palette.foreground)); err != nil {
		return err
	}
	...
	if _, err := fmt.Fprintf(r.w, `<text x="%s" y="%s"%s>`, formatFloat(x), formatFloat(textY), r.fillToken(r.palette.background)); err != nil {
		return err
	}
