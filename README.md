...
If `-o` is a directory, the animation file is named `ttyanim_YYYY.MM.DD-HH.MM.SS.svg`. By default, output is written to that timestamped name in the directory where `ttysvg` was started. Pane-mode snapshots use `ttypic_YYYY.MM.DD-HH.MM.SS.svg` in the same directory.
